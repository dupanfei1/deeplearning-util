...
work_path = 'train'                # 存放要工作文件夹名
work_dir_name = '1'                       # 存放图像文件的文件夹名
convert_image_dir_name = 'train_convert_image'  # 存放转化后图片的文件夹名
...
for j in range(1,7):
    work_dir_path= work_dir_path1+ str(j)
    convert_image_dir_path=convert_image_dir_path1+str(j)
    print j
    ...
    for x in image_file_name_path:
        ...
        if GAUSSIAN_BLUR:
            ...
            if j== 6:
                img_gb = img.filter(MyGaussianBlur(radius=3))  # 对图像进行高斯模糊，模糊半径为4
                img_gb.save(convert_image_dir_path + os.sep + 'gb_3_' + os.path.basename(x))
        if TRANSPOSE_FLAGE:                                                            # 对图像进行旋转
            img_rotate_20 = img.rotate(20)                                             # 对图像旋转30度
            img_rotate_negative_30 = img.rotate(-20)                                   # 对图像旋转负30度
            img_rotate_20.save(convert_image_dir_path + os.sep + 'rotate_20_' + os.path.basename(x))
        ...
        if LIGHT_FLAGE:                                                                # 对图像进行亮度增强
            ...
            if j==6:
                brightness = 2
                img_brightened = enh_bri.enhance(brightness)
                img_brightened.save(convert_image_dir_path + os.sep + 'light_3_' + os.path.basename(x))

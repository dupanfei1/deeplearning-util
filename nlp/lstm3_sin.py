...
#------------------------------------Generate Data-----------------------------------------------#
#generate data
def generate(seq):  #划分训练和测试集，每10个点预测第11个点的值
    X=[]
    y=[]
...
seq_train=np.sin(np.linspace(start=0,stop=100,num=TRAIN_EXAMPLES,dtype=np.float32))#0到100，11000个点
seq_test=np.sin(np.linspace(start=100,stop=110,num=TEST_EXAMPLES,dtype=np.float32))#要预测的100-110,1100个点
...
with graph.as_default():
    ...
    #------------------------------------construct LSTM------------------------------------------#
    #place hoder
    X_p=tf.placeholder(dtype=tf.float32,shape=(None,TIME_STEPS,1),name="input_placeholder")#None表示batchsize,timestep为输入点数10。
    y_p=tf.placeholder(dtype=tf.float32,shape=(None,1),name="pred_placeholder")

    #lstm instance
    lstm_cell=rnn.BasicLSTMCell(num_units=HIDDEN_UNITS)#输出１个点
